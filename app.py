...
def train_models(dataframe: pd.DataFrame):
    # If pickled models exist, load them and compute metrics; else, train fresh
...
with tab1:
    ...
    with c2:
        ...
        st.markdown("**Sales (mg/L)**")
        filt = {}
        for f, levels in DISCRETE_LEVELS.items():
            sel = st.multiselect(f, options=levels, default=[])
            if len(sel)>0: filt[f] = sel
    ...
    with c1:
        ...
        with colA:
            fig = px.histogram(df, x="protein", color="Species", nbins=30, barmode="overlay", histnorm="probability density", title="Protein (%)")
...
with tab2:
    ...
    with left:
        model_choice = st.selectbox("Modelo", options=list(models.keys()), index=list(models.keys()).index("RandomForest") if "RandomForest" in models else 0)
        st.markdown("**Entradas**")
        inputs = {}
        for k, levels in DISCRETE_LEVELS.items():
            if k == "luminosity_umol_m2_s":
                inputs[k] = st.selectbox(k, options=levels, index=levels.index(650) if 650 in levels else 0)
            else:
...
with tab3:
    ...
    with colA:
        model_choice2 = st.selectbox("Modelo", options=list(models.keys()), index=list(models.keys()).index("RandomForest") if "RandomForest" in models else 0, key="mdl2")
    ...
    with colB:
        ...
        fig = px.line(pdf, x="x", y="protein_pred", markers=True, title=f"Respuesta de {var_to_sweep} – {model_choice2}")
        fig.update_layout(xaxis_title=var_to_sweep, yaxis_title="protein pred (%)")
        st.plotly_chart(fig, use_container_width=True)

st.markdown("---")
st.caption("MVP construido con Streamlit · CSV originales + modelos pre-entrenados (si están en /models).")
